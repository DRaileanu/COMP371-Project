...
{
    icosahedronVertices.push_back(x);
    icosahedronVertices.push_back(y);
    icosahedronVertices.push_back(z);
}
...
{
    icosahedronTexCoords.push_back(s);
    icosahedronTexCoords.push_back(t);
}
...
{
    icosahedronTexCoords.push_back(t1[0]); // s
    icosahedronTexCoords.push_back(t1[1]); // t
    icosahedronTexCoords.push_back(t2[0]);
    icosahedronTexCoords.push_back(t2[1]);
    icosahedronTexCoords.push_back(t3[0]);
...
void Sphere::subdivideVerticesFlat()
{
    ...
    for (i = 1; i <= subdivision; ++i)
    {
        ...
        tmpTexCoords = icosahedronTexCoords;
        tmpIndices = indices;
...
void Sphere::subdivideVerticesSmooth()
{
    ...
    for (i = 1; i <= subdivision; ++i)
    {
        ...
        for (j = 0; j < indexCount; j += 3)
        {
            ...
            v1 = &icosahedronVertices[i1 * 3];
            v2 = &icosahedronVertices[i2 * 3];
            v3 = &icosahedronVertices[i3 * 3];
            t1 = &icosahedronTexCoords[i1 * 2];
            t2 = &icosahedronTexCoords[i2 * 2];
            t3 = &icosahedronTexCoords[i3 * 2];
...
{
    // normalize
    float scale = Sphere::computeScaleForLength(v, 1);
    normal[0] = v[0] * scale;
    normal[1] = v[1] * scale;
    normal[2] = v[2] * scale;
}
...
bool Sphere::isSharedTexCoord(const float t[2])
{
    ...
    {
        if (Sphere::isOnLineSegment(&segments[i], &segments[j], t))
